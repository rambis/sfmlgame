...
{
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && bulletShot == false)
	{
		bulletShot = true;
		return true;
	}
...
void gameLoop() 
{
	...
	sf::RenderWindow window(videoMode, "Destructo");
	...
	strcat_s(bulletImage, "testbullet.png");
	...
	std::vector <SingleBullet> bulletVec;
	int count = 0;
	...
	while (window.isOpen())
	{
		...
		std::vector <SingleBullet>::iterator it = bulletVec.begin();

		//if (sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left)
			//bulletShot = false;

		if (keyPressDebounce(bulletShot))
		{
			SingleBullet bullet;
			bullet.setBulletTexture(bulletTexture);
			//	bullet.setBulletTextureRect(sf::IntRect(85, 72, 21, 22));
			bullet.setBulletTextureRect(sf::IntRect(0,0,15,15));
			sf::Vector2f playerPosition = myPlayer.getPosition();
			bullet.setBulletPosition(playerPosition, (direction)source.y);
			bullet.setBulletShot(true);
			bullet.setDirection((direction)source.y);
			bulletVec.push_back(bullet);
		}

		it = bulletVec.begin();
		while (it != bulletVec.end()) {
			sf::Vector2f bulletPos = it->getBulletPosition();
			if (bulletPos.x > screenRes.x || bulletPos.x < 0 || bulletPos.y < 0 || bulletPos.y > screenRes.y)
				it = bulletVec.erase(it);
			else {
				it->moveBullet(screenRes);
			}
			if (it == bulletVec.end())
				break;
			it++;
		...
		/*std::vector <sf::Sprite>::iterator } = bulletVec.begin();
